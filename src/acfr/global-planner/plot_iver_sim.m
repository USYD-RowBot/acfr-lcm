...
while 1
    ...
    path_counter = t3-1;
    color = 0;
    while paths_shown < 10
        if x3(path_counter) == 0
            paths_shown = paths_shown + 1;
            plot3(path_save(1,1:path_save_counter-1),path_save(2,1:path_save_counter-1),path_save(3,1:path_save_counter-1),'Color',[1-color color color])
            color = color + 0.05;
            path_save_counter = 1;
        else
