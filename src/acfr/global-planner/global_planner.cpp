...
void onPathResponse(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const acfrlcm::auv_path_response_t *pr, GlobalPlanner* gp) {
...
void onGlobalPlannerCommand(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const acfrlcm::auv_global_planner_t *gm, GlobalPlanner* gp) {
...
GlobalPlanner::GlobalPlanner() : globalPlannerMessage(globalPlannerIdle) {
...
string GlobalPlanner::getCurrentStateString() {
    string GlobalPlannerStateStr[] = {"idle", "run", "abort", "pause", "done", "fault"};
...
int GlobalPlanner::sendLeg()
{
    ...
    cout << "Sending way point " << (*currPoint).id << ": " << (*currPoint).pose.getX() << ", " << (*currPoint).pose.getY() << ", " << (*currPoint).pose.getZ() << endl;
