...
class MissionPrimitive {
	...
	public:
        ...
        bool generatePath(double _timeout, depthModeT _depthMode, list<MissionCommand> &commands, int wpStartNum=0) {
		    timeout = _timeout;
		...
		void setDepthMode( depthModeT d ) { depthMode = d; }
		void setMissionGoalType( missionGoalTypeT m ) { goalType = m; }
		...
		std::list<waypoint> getPath( void ) const { return path; }
		...
		bool setMinWidthLength( double m ) { if( m < 0 ) return false; minWidthLength=m; return true; }
		
		void setAltitudeChange( double a ) { altitudeChange = a; }
		void setNumLoops( unsigned int n ) { numLoops = n; }
