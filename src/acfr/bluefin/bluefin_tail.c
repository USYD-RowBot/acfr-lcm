...
void nga_motor_command_handler(const lcm_recv_buf_t *rbuf, const char *ch, const acfrlcm_auv_nga_motor_command_t *mot, void *u)
{
    ...
    {
        state->thruster = state->thruster*0.2; //if we see a brown out coming then limit the thruster values to 50% requested. At max rpm of 600 this will be limited to 300rpm
