...
const int DubinsPath2D::side[2] = {right, left};


/** **************************************************
 *  Constructor
 */
DubinsPath2D::DubinsPath2D() :
		minDistI(0), minDist(0), minSide1(0), minSide2(0), circleRad(
		1.0), dropDist(1.0), angle1_report(0), angle2_report(
				0) {
...
DubinsPath2D::DubinsPath2D(double _circRad, double _dropDist,
		double _dropAngle) :
		minDistI(0), minDist(0), minSide1(0), minSide2(0), circleRad(
				_circRad), dropDist(_dropDist), dropAngle(
 ...
 */
vector<Pose2D> DubinsPath2D::calcPath(Pose2D currPose, Pose2D destPose) {
...
//		cout << "Source circle on " << sideStr[i] << endl;
		...
		cSrc.setThetaRad(currPose.getThetaRad() - M_PI / 2);
...
//			cout << "Dest circle on " << sideStr[j] << endl;
			...
			cDest.setThetaRad(destPose.getThetaRad() - M_PI / 2);
...
//					<< ", lineC2CLen " << lineC2CLen << endl;
			...
			p1.setPosition(cSrc.getX() + dX, cSrc.getY() - dY);
			p1.setThetaRad(lineC2CTheta + M_PI / 2);
			q1.setPosition(cSrc.getX() - dX, cSrc.getY() + dY);
			q1.setThetaRad(lineC2CTheta + M_PI / 2);
			p2.setPosition(cDest.getX() + dX, cDest.getY() - dY);
			p2.setThetaRad(lineC2CTheta + M_PI / 2);
			q2.setPosition(cDest.getX() - dX, cDest.getY() + dY);
			q2.setThetaRad(lineC2CTheta + M_PI / 2);
...
//			cout << "target " << pose2[run].toString() << endl;
			...
			dist = 0;
			// First Turn
			double angleDiff1 = (connectionLineTheta - currPose.getThetaRad())
					* side[i];
			while (angleDiff1 < 0) {
...
//					<< angleDiff1 / M_PI * 180 << endl;
			...
			// Second Turn
			double angleDiff2 = (destPose.getThetaRad() - connectionLineTheta)
					* side[j];
			while (angleDiff2 < 0) {
...
//			cout << "Total         : " << dist << "m" << endl;
	...
	// Calc the limits
	double angle1 = (minPose1.getThetaRad() - currPose.getThetaRad()) * minSide1;
	while (angle1 < 0) {
		angle1 += 2 * M_PI;
	}
	double straightDist = minPose1.positionDistance(minPose2);
	double angle2 = (destPose.getThetaRad() - minPose2.getThetaRad()) * minSide2;
	while (angle2 < 0) {
		angle2 += 2 * M_PI;
	}
	...
	double heading = currPose.getThetaRad();
	double distance = 0;

	// Circle 1 path
	minC1.setThetaRad(atan2(currPose.getY() - minC1.getY(),
	...
	for (double tT = dropAngle; tT < angle1; tT += dropAngle) {
		...
		pRel.setPosition(this->circleRad * cos(tT * minSide1),
				this->circleRad * sin(tT * minSide1));
	...
	// Circle 2 path
	minC2.setThetaRad(atan2((*(path.end() - 1)).getY() - minC2.getY(),
	...
	for (double tT = dropAngle; tT < angle2; tT += dropAngle) {
		...
		pRel.setPosition(this->circleRad * cos(tT * minSide2),
				this->circleRad * sin(tT * minSide2));
