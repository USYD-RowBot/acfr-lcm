...
int homography_estimator::ransac(homography06 * H, const float threshold, const int maximum_number_of_iterations,
                                 const float P, bool prosac_sampling, double * inlier_rms_rproj_error)
{
  ...
  number_of_inliers = compute_inliers(H, inliers, threshold, inlier_rms_rproj_error);
...
int homography_estimator::compute_inliers(homography06 * H, bool * inliers, float threshold, double * inlier_rms_rproj_error)
{
  int n = 0;
  double sq_rproj_error = 0.0;
  double sq_rproj_error_sum = 0.0;

  for(int i = 0; i < number_of_correspondences; i++) {
    float eup, evp;
    H->transform_point(u_v_up_vp[4 * i], u_v_up_vp[4 * i + 1], eup, evp);
    sq_rproj_error = ((u_v_up_vp[4 * i + 2] - eup) * (u_v_up_vp[4 * i + 2] - eup) +
                  (u_v_up_vp[4 * i + 3] - evp) * (u_v_up_vp[4 * i + 3] - evp));
    inliers[i] = sq_rproj_error < threshold * threshold;

    if (inliers[i]) 
    { 
	n++;
	sq_rproj_error_sum += sq_rproj_error;
    }
  }

  if(inlier_rms_rproj_error != NULL){
      sq_rproj_error_sum = sq_rproj_error_sum / n;
      *inlier_rms_rproj_error = sqrt(sq_rproj_error_sum);
  }
