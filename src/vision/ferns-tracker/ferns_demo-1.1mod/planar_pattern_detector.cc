...
bool planar_pattern_detector::detect(const IplImage * input_image, homography06 *myH)
{
  ...
  if (pattern_is_detected) {
	...
	if (dist2 > 10.0 * 10.0)
      ...
      (*myH).cvmSet(0,0,H.cvmGet(0,0));(*myH).cvmSet(0,1,H.cvmGet(0,1));(*myH).cvmSet(0,2,H.cvmGet(0,2));
      (*myH).cvmSet(1,0,H.cvmGet(1,0));(*myH).cvmSet(1,1,H.cvmGet(1,1));(*myH).cvmSet(1,2,H.cvmGet(1,2));
      (*myH).cvmSet(2,0,H.cvmGet(2,0));(*myH).cvmSet(2,1,H.cvmGet(2,1));(*myH).cvmSet(2,2,H.cvmGet(2,2));
      
  }
  number_of_inliers = H_estimator->number_of_inliers;
  number_of_correspondences = H_estimator->number_of_correspondences;
...
bool planar_pattern_detector::estimate_H(void)
{
  ...
  return H_estimator->ransac(&H, 10., 1500, 0.99, true, &inlier_rms_rproj_error) > 10;
}
