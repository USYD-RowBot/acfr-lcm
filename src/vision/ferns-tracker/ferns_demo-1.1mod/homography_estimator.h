...
class homography_estimator
{
 public:
  ...
  int ransac(homography06 * H, const float threshold, const int maximum_number_of_iterations,
             const float P = 0.99, bool prosac_sampling = true, double * inlier_rms_rproj_error = NULL);
  ...
  int compute_inliers(homography06 * A, bool * inliers, float threshold, double * inlier_rms_rproj_error = NULL);
