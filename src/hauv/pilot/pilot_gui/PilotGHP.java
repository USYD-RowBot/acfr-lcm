...
public class PilotGHP implements Runnable {
            ...
            while (nextWaypt == 0 && flag == 1) {
                ...
                */
				...
				if (sendCount == 0) {
					...
					ghp.horizontal = ycommand;
					ghp.vertical = -xcommand;
					//ghp.relative_bearing = 0;//-thetaDrift; //Change back to zero if errors occur
					//ghp.is_depth = false; //true
					ghp.distance = des_depth; //Standoff from hull!
					sendCount = 0;
				}
				myLCM.publish ("HAUV_PL_SUS", sus);
				...
				if (holdStation == true) {
					...
					if((Math.abs(x_dvl-xwaypt) < xdiff) && 
					   (Math.abs(y_dvl-ywaypt) < ydiff))
