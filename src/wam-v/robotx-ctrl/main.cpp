...
typedef struct
{
    ...
    int run_mode; // TODO Need to get this from ROS now - create a ros publisher + bridge
...
void left_ros_callback(const lcm::ReceiveBuffer *rbuf, const std::string& channel,
                        const acfrlcm::asv_torqeedo_motor_command_t *ros_cmd, state_t *state)
{
    // update state
    state->prev_control_time = ros_cmd->utime;
    state->left_motor_cmd = ros_cmd->command_speed;
    if(ros_cmd->enabled) // TODO check this works with overall logic
    {
        state->run_mode = acfrlcm::wam_v_control_t::RUN;
    }
    else
    {
...
void right_ros_callback(const lcm::ReceiveBuffer *rbuf, const std::string& channel,
                        const acfrlcm::asv_torqeedo_motor_command_t *ros_cmd, state_t *state)
{
    // update state
    state->prev_control_time = ros_cmd->utime;
    state->right_motor_cmd = ros_cmd->command_speed;
    if(ros_cmd->enabled) // TODO check this works with overall logic
    {
        state->run_mode = acfrlcm::wam_v_control_t::RUN;
    }
    ...
    else if ((state->control_source == RC_MODE_AUTO) && (!in_boot_delay))
    {
        ...
        else // no timeout
        {
            if (state->run_mode == acfrlcm::wam_v_control_t::RUN)
            {
                ...
                mc_port.command_speed = state->left_motor_cmd;
                mc_stbd.command_speed = state->right_motor_cmd;
...
int main(int argc, char **argv)
{
    ...
    // LCM callbacks
    state.lcm.subscribeFunction(state.vehicle_name+".LEFT_ROS_CMD", left_ros_callback, &state); // The ROS Bridge should publish these messages
    state.lcm.subscribeFunction(state.vehicle_name+".RIGHT_ROS_CMD", right_ros_callback, &state); // The ROS Bridge should publish these messages
