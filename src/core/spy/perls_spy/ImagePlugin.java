...
public class ImagePlugin implements lcm.spy.SpyPlugin
{
	...
	BufferedImage handleRAW16(image_t v)
	{
	    BufferedImage bi = new BufferedImage(v.width, v.height, BufferedImage.TYPE_INT_RGB);
	    for (int y = 0; y < v.height; y++) {
		for (int x = 0; x < v.width; x++) {
		    int a = v.data[y*v.row_stride + x*2 + 0] & 0xff;
		    int b = v.data[y*v.row_stride + x*2 + 1] & 0xff;
		    int rgb = (b<<16) | (b<<8) | b;
		    bi.setRGB(x, y, rgb);
		}
	    }
	...
	BufferedImage handleGRAY16(image_t v, boolean LE)
	{
	    BufferedImage bi = new BufferedImage(v.width, v.height, BufferedImage.TYPE_INT_RGB);
	    for (int y = 0; y < v.height; y++) {
                for (int x = 0; x < v.width; x++) {
	...
	BufferedImage handleRGB16(image_t v, boolean LE)
	{
	    BufferedImage bi = new BufferedImage(v.width, v.height, BufferedImage.TYPE_INT_RGB);
	    for (int y = 0; y < v.height; y++) {
		for (int x = 0; x < v.width; x++) {
                    int ra = v.data[y*v.row_stride + x*6 + 0] & 0xff;
                    int rb = v.data[y*v.row_stride + x*6 + 1] & 0xff;
                    int ga = v.data[y*v.row_stride + x*6 + 2] & 0xff;
                    int gb = v.data[y*v.row_stride + x*6 + 3] & 0xff;
                    int ba = v.data[y*v.row_stride + x*6 + 4] & 0xff;
                    int bb = v.data[y*v.row_stride + x*6 + 5] & 0xff;
                    int rgb;
                    if (LE)
                        rgb = (rb<<16) | (gb<<8) | bb;
                    else
	...
	public void handleImage(image_t v)
	{
		...
		case image_t.PIXEL_FORMAT_RGB:
		    bi = handleRGB(v);
		    break;
		case image_t.PIXEL_FORMAT_LE_GRAY16:
                case image_t.PIXEL_FORMAT_LE_BAYER16_BGGR:
                case image_t.PIXEL_FORMAT_LE_BAYER16_GBRG:
                case image_t.PIXEL_FORMAT_LE_BAYER16_GRBG:
                case image_t.PIXEL_FORMAT_LE_BAYER16_RGGB:
                    bi = handleGRAY16(v,true);
                    break;
		case image_t.PIXEL_FORMAT_BE_GRAY16:
                case image_t.PIXEL_FORMAT_BE_BAYER16_BGGR:
                case image_t.PIXEL_FORMAT_BE_BAYER16_GBRG:
                case image_t.PIXEL_FORMAT_BE_BAYER16_GRBG:
                case image_t.PIXEL_FORMAT_BE_BAYER16_RGGB:
                    bi = handleGRAY16(v,false);
                    break;
                case image_t.PIXEL_FORMAT_BE_RGB16:
                    bi = handleRGB16(v,false);
                    break;
                case image_t.PIXEL_FORMAT_LE_RGB16:
                    bi = handleRGB16(v,true);
                    break;
                default:
                    System.out.println("ImagePlugin: unsupported image format");
                    break;
                }
