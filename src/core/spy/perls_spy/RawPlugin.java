...
public class RawPlugin implements lcm.spy.SpyPlugin
{
    public boolean canHandle(long fingerprint)
    {
        if (fingerprint == senlcm.raw_t.LCM_FINGERPRINT)
            return true;
        else
            return false;
    }
    ...
    class Viewer extends JInternalFrame implements lcm.lcm.LCMSubscriber
    {
    	...
    	public Viewer(ChannelData cd, String menu_name)
    	{
            super(cd.name+": Raw Data Viewer", true, true);
            this.cd = cd;

            setLayout(new BorderLayout());
            op = new myObjectPanel(cd.name);
    ...
    public class myObjectPanel extends JPanel
    {
        String name;
        Object o;
        int lastwidth = 400;
        ...
        public myObjectPanel(String name)
        {
            this.name = name;
        ...
        public void paint(Graphics g)
        {
            ...
            if (ps.x[2] + ps.maxwidth > lastwidth) {
                lastwidth = ps.x[2] + ps.maxwidth + 10;
                lastheight = ps.y;
                invalidate();
                getParent().validate();
            }
            ...
            if (cls.equals(Byte.TYPE)) {
                ps.drawStrings(cls.getName(), name,
                               String.format("0x%02X   %03d   %+04d   %c",
                                             ((Byte)o),((Byte)o).intValue()&0x00FF,((Byte)o),((Byte)o)&0x00FF),
                               isstatic);
                ...
                ps.drawStrings("String", name, s, isstatic);
                ps.maxwidth = Math.max(ps.maxwidth, ps.fm.stringWidth(s));
