...
int read_inertial_frame(int portFd, unsigned char* frame_buf){
    ...
    while(1) {
        ...
        switch(MsgID){
        case CTRL_MSG: Nbytes = FLIGHT_CTRL_SIZE + CHECKSUM_SIZE; break;    
        case INERT_MSG: Nbytes = INERTIAL_SIZE + FLIGHT_CTRL_SIZE + CHECKSUM_SIZE; break;
        default: 
