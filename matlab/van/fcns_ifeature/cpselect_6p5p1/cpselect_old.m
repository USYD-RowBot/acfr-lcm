...
%   grayscale or RGB images or strings that identify files containing
...
%
%   H = CPSELECT(INPUT,BASE,...) returns a handle H to the tool. DISPOSE(H)
...
%
%   See also CPCORR, CP2TFORM, IMTRANSFORM.
%
%   ******************* NOTE *******************************
%   10/17/2003  rme   Modified to accept color RGB images.
  
%   Copyright 1993-2002 The MathWorks, Inc.
%   $Revision: 1.31 $  $Date: 2002/04/12 16:12:19 $
...
if DEBUG_CPSELECT & strcmpi(computer,'sgi')
    disp('Inside cpselect, about to call javachk.');
end 

% Don't run on platforms with incomplete Java support
eid = sprintf('Images:%s:noJavaSupport',mfilename);
msg = javachk('mwt','CPSELECT');
if ~isempty(msg)
  error(eid,msg);
end
...
baseImageName = get_Imageitle(varargin{2},inputname(2));

inputImage = im2java(input);
baseImage = im2java(base);

% import Java classes used here
import com.mathworks.toolbox.images.ControlPointSelectionTool;
...
if ischar(arg)
    try 
        info = imfinfo(arg);
        %rme if strmatch(info.ColorType,'indexed')
        %rme    eid = sprintf('Images:%s:imageMustBeGrayscale',mfilename);
        %rme    msg = sprintf('%s: %s must be a grayscale image.',upper(mfilename),arg);
        %rme    error(eid,msg);
        %rme end
        img = imread(arg);
    catch
...
%rme checkinput(img,{'uint8','uint16','double','logical'},{'real','nonsparse','2d'},...
%rme           mfilename,argname,argnumber);
% rme
%vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
checkinput(img,{'uint8','uint16','double','logical'},{'real','nonsparse'},...
...
import com.mathworks.toolbox.images.CPVector;
import com.mathworks.toolbox.images.ControlPoint;
import com.mathworks.toolbox.images.ControlPointPair;
