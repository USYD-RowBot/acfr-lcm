function secs_from_1970 = unixtime(year,month,day,hours,minutes,seconds)
%UNIXTIME Converts Y/M/D H:M:S to unix time.
...
ii = find(year < 50);
if ~isempty(ii)
    year(ii) = year(ii) +2000;
end
ii = find(year < 100);
if ~isempty(ii)
    year(ii) = year(ii) + 1900;
end
