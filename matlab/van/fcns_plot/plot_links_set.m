function varargout = plot_links_set(dof,param_t,mu,Sigma,vlinks,Index,TheConfig,Handles);
%PLOT_LINKS
%function varargout = plot_links_set(dof,param_t,mu,Sigma,vlinks,Index,TheConfig,Handles);
%
...
%
%   Handles is an optional input/output graphics handles structure
%-----------------------------------------------------------------
%    History:
%    Date            Who         What
%    -----------     -------     -----------------------------
...
%================================
% ALLOCATE NEW GRAPHICS HANDLES
%================================
% ellipse
m = size(ellipseX,2);
n = m-length(Handles.ellipse);
init = zeros(2,n);
Handles.ellipse(end+1:m,1) = line(init,init);

% center
m = size(ellipseX,2);
n = m-length(Handles.center);
init = zeros(2,n);
Handles.center(end+1:m,1) = line(init,init);

% temporal links
m = length(tlinkX);
n = m-length(Handles.tlinks);
init = zeros(2,n);
Handles.tlinks(end+1:m,1) = line(init,init);

% spatial links
m = length(slinkX);
n = m-length(Handles.slinks);
init = zeros(2,n);
Handles.slinks(end+1:m,1) = line(init,init);
...
case 1
 ...
 if dof == 2;
   set(Handles.tlinks,'xdata',tlinkX,'ydata',tlinkY,'color',param_t.tl_color,'LineWidth',param_t.linewidth);
   set(Handles.slinks,'xdata',slinkX,'ydata',slinkY,'color',param_t.sl_color,'LineWidth',param_t.linewidth);
 elseif dof == 3;
   set(Handles.tlinks,'xdata',tlinkX,'ydata',tlinkY,tlinkZ,'color',param_t.tl_color,'LineWidth',param_t.linewidth);
   set(Handles.slinks,'xdata',slinkX,'ydata',slinkY,slinkZ,'color',param_t.sl_color,'LineWidth',param_t.linewidth);
 end;
case 2
 % show only current image links
 if dof == 2;
   set(Handles.tlinks,'xdata',tlinkX(:,end),'ydata',tlinkY(:,end),'color',param_t.tl_color,'LineWidth',param_t.linewidth);
   set(Handles.slinks,'xdata',slinkX(:,csi),'ydata',slinkY(:,csi),'color',param_t.sl_color,'LineWidth',param_t.linewidth);   
 elseif dof == 3;
   set(Handles.tlinks,'xdata',tlinkX(:,end),'ydata',tlinkY(:,end),tlinkZ(:,end),'color',param_t.tl_color,'LineWidth',param_t.linewidth);
   set(Handles.slinks,'xdata',slinkX(:,csi),'ydata',slinkY(:,csi),slinkZ(:,csi),'color',param_t.sl_color,'LineWidth',param_t.linewidth);
 end;
otherwise
...
if param_t.colorscaled;
  ...
  if dof == 2;
    % plot delayed state ellipses
    set(Handles.ellipse,'xdata',ellipseX(:,1:end-1),'ydata',ellipseY(:,1:end-1), ...
		      'Color',param_t.ds_color,'MarkerSize',param_t.markersize);
    hold on;
    set(Handles.ellipse(end+1),'xdata',ellipseX(:,end),'ydata',ellipseY(:,end), ...
		      'Color',param_t.cs_color,'MarkerSize',param_t.markersize);
    % plot ellipse centers
    set(Handles.center,'xdata',posX(1:end-1),'ydata',posY(1:end-1),'+', ...
		      'Color',param_t.ds_color,'MarkerSize',param_t.markersize);
    set(Handles.center(end+1),'xdata',posX(end),'ydata',posY(end),'+', ...
		      'Color',param_t.cs_color,'MarkerSize',param_t.markersize);
  elseif dof == 3;
    n = sqrt(size(ellipseX,1));
    hold on;
