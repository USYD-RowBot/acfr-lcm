...
%SUBIMAGESC Display multiple images in single figure.
...
%   $Revision: 5.29 $  $Date: 2003/02/18 17:16:36 $
...
case 3
    ...
    elseif (isequal(size(varargin{1}), size(varargin{2})) & ...
        ...
        % subimage(R,G,B)
        warning(['SUBIMAGE(R,G,B) is an obsolete syntax. ',...
...
case 5
    ...
    if (~isequal(size(varargin{3}), size(varargin{4})) | ...
        ...
        % subimage(x,y,R,G,B)
        warning(['SUBIMAGE(x,y,R,G,B) is an obsolete syntax. ',...
        'Use a three-dimensional array to represent RGB image.']);
        x = varargin{1};
        y = varargin{2};
