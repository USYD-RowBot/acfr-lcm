function [] = update_aug_eif(meas_t,TheConfig)
%function [] = update_aug_eif(meas_t,TheConfig)
%-----------------------------------------------------------------
%    History:
%    Date            Who         What
...
TheJournal.Eif.eta(nzi) = eta(nzi) + Haug(:,nzi)'*R_inv*nu;

% information matrix update
%---------------------------------------------
TheJournal.Eif.Lambda(nzi,nzi) = Lambda(nzi,nzi) + spdproduct(R_inv,Haug(:,nzi));
