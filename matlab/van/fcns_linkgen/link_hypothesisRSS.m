function [fni,fnj,probability,mu_d,sigma_dd,altratio] = link_hypothesisRSS(TheConfig);
...
confidence = 0.10 ;       % confidence in overlap [0 1]
...
for fni=Ni:Nf-1; % loop over all other images
    ...
    % i.e. d(mu_d)/d(x_lci(1:2),xlcj(1:2)) * d(x_lci(1:2),xlcj(1:2))/d(x_lvi,x_lvj,x_vc)
...
[junk,ii] = sortrows([-roundunit(probability,0.10), roundunit(mu_d,0.5), roundunit(abs(1-altratio),0.10)]);
fni = fni(ii);
fnj = fnj(ii);
mu_d = mu_d(ii);
