function data = test_algorithm_unixtime(nav_t)
clc
if ~isfield(nav_t, 'ES.elaptime')
    ...
    nav_t.RDI.dt = [0; diff(nav_t.RDI.elaptime)];
    ...
    nav_t.ES.unixtime = nav_t.TRITECH_ES.unixtime;
    nav_t.ES.elaptime = nav_t.TRITECH_ES.elaptime;
    nav_t.ES.x        = interp1(nav_t.RDI.unixtime, nav_t.RDI.xyz(:,1), ...
        nav_t.ES.unixtime);
    nav_t.ES.y        = interp1(nav_t.RDI.unixtime, nav_t.RDI.xyz(:,2), ...
        nav_t.ES.unixtime);
    nav_t.ES.depth    = interp1(nav_t.DESERT_STAR.unixtime, ...
        nav_t.DESERT_STAR.depth, nav_t.ES.unixtime);
    nav_t.ES.dx       = [0; diff(nav_t.ES.x)];
%     nav_t.ES.alt      = interp1(nav_t.RDI.unixtime, nav_t.RDI.altitude, ...
%         nav_t.ES.unixtime);
    nav_t.ES.alt      = interp1(nav_t.OS_CONDUIT_OSI.unixtime, ...
        nav_t.OS_CONDUIT_OSI.altimeter, nav_t.ES.unixtime);
    nav_t.ES.bottom_y = nav_t.ES.depth + nav_t.ES.alt;
    nav_t.ES.bottom_x = nav_t.ES.x + 5*sin(pi/6);
    nav_t.ES.pitch    = interp1(nav_t.MICROSTRAIN.unixtime, ...
        nav_t.MICROSTRAIN.sEuler(:,2), nav_t.ES.unixtime);
...
count = 1;
% aviobj = avifile('TB-current2.avi');
% fig = figure;
